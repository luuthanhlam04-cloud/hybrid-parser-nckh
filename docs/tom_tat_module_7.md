...
- **Tình trạng**: Ở M6, LLM trả về các type tự do như `SUBJECT`, `ACTION`, `OBJECT`. Khi rơi vào Quarantine, thực thể giữ nguyên type cũ. Kết quả là `OntologyValidator` của M7 đã **từ chối 100%** các Cạnh nối với thực thể Quarantine vì lỗi sai Domain-Range. Nguy hiểm hơn, M6 trích xuất Khách thể là `OBJECT`, nhưng M7 (ban đầu) lại map nhầm `OBJECT` thành `LEGAL_SUBJECT`, trong khi Taxonomy V1.4 đã định nghĩa là `LEGAL_OBJECT`. Việc lệch pha này khiến toàn bộ các thực thể hợp lệ như "Quyền sử dụng đất", "Giấy chứng nhận" bị Quarantine oan uổng.
- **Giải pháp**: Tại hàm `_quarantine` trong `entity_normalizer.py`, tôi đã lập tức thiết lập một bộ `M6_TO_M7_MAP` để ngầm ánh xạ type một cách chính xác nhất (`SUBJECT` -> `LEGAL_SUBJECT`, `ACTION` -> `LEGAL_ACTION`, và đặc biệt sửa lỗi map `OBJECT` -> `LEGAL_OBJECT`). Nhờ đó, các thực thể hợp lệ lập tức được ánh xạ thành công, cứu sống hàng trăm cạnh trên đồ thị.

### Vấn đề 2: Thiếu Quan hệ `HAS_OBJECT` trong Ma trận Ràng buộc
- **Tình trạng**: Khi chia tách `LEGAL_SUBJECT` và `LEGAL_OBJECT` trong Taxonomy V1.4, các cạnh nối từ Hành vi sang Khách thể (Vd: Nhận chuyển nhượng -> Quyền sử dụng đất) mang nhãn `HAS_OBJECT` đã bị Validator ném ra lỗi `UNKNOWN_M6_RELATION` và loại bỏ 85 cạnh oan uổng, vì `ontology_schema.yaml` chưa hề khai báo quan hệ này.
- **Giải pháp**: Bổ sung khai báo `HAS_OBJECT` vào `ontology_schema.yaml` với Domain là `[LEGAL_ACTION, LEGAL_SUBJECT]` và Range là `[LEGAL_OBJECT, LEGAL_SUBJECT]`. Ngay lập tức, Validator tự động học được luật mới (không cần sửa code Python) và tiếp nhận lại 85 cạnh này.

### Vấn đề 3: Xung đột giữa "Gộp mảng ngữ cảnh" và "Thực thể siêu nhẹ" (Edge-Level Provenance)
...
Nhờ việc sửa lỗi ép kiểu `OBJECT` thành `LEGAL_OBJECT` và bổ sung các alias còn thiếu vào `taxonomy_aliases.yaml` (V1.4 - 873 dòng), Đồ thị Tri thức đã có bước nhảy vọt:
- **Canonical Entities:** Tăng từ 52 lên **70** thực thể.
- **Canonical Relations:** Tăng vọt từ 175 lên **281** quan hệ hợp lệ.
- **Quarantined Entities:** Giảm từ 433 xuống còn **256** thực thể.
Đặc biệt, hệ thống đã chứng minh được năng lực tự làm sạch (Self-Cleaning) xuất sắc khi thẳng tay loại bỏ 67 cạnh `ALLOW` nối trực tiếp vào `LEGAL_OBJECT` (Action-Object Fallacy do LLM sinh ra). Mọi rác rưởi ảo giác đều bị Validator cách ly tuyệt đối!
